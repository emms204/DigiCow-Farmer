#!/usr/bin/env python3
"""
E10.4 — XGBoost HPO with vetted features (single model per target).

Same protocol as E10.2: one model per target, weighted + 7d LL. Guardrails:
7d_ll, worst_fold, fold_std, last_fold not worse. Pass rule: E10.4 best > E10.1 best.

HPO: max_depth, min_child_weight, subsample, colsample_bytree, reg_alpha,
reg_lambda, eta, gamma, n_estimators (logloss objective).

Usage:
  python e10_4_xgb_hpo.py [--run-baseline-if-missing] [--study 7d|all] [--n-trials 30]
  python e10_4_xgb_hpo.py --e10-1-best 0.893
"""
...
from e10_dual_hpo import (
    E10_N_SPLITS,
    E10_SEED,
    run_single_e10_harness_xgb,
    _default_xgb_ll_params,
)
...
E10_4_BASELINE_PATH = Path(__file__).resolve().parent / "e10_4_baseline.json"
E10_4_BEST_PATH = Path(__file__).resolve().parent / "e10_4_best.json"
E10_1_BEST_PATH = Path(__file__).resolve().parent / "e10_1_best.json"


def _load_baseline() -> dict[str, Any]:
    if not E10_4_BASELINE_PATH.exists():
        raise FileNotFoundError(
            f"E10.4 baseline not found at {E10_4_BASELINE_PATH}. "
            "Run with --run-baseline-if-missing first."
        )
    return json.loads(E10_4_BASELINE_PATH.read_text(encoding="utf-8"))
...
def _run_and_save_baseline_once(
    ...
    """One E10.4 XGBoost harness run; save as baseline."""
    logger.info("Running one E10.4 XGBoost harness to create baseline at %s ...", E10_4_BASELINE_PATH)
    result = run_single_e10_harness_xgb(
    ...
    E10_4_BASELINE_PATH.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    logger.info("  Baseline saved: weighted=%.6f", result["weighted_score"])
    return payload


def _load_reference_score(args: argparse.Namespace) -> float:
    """E10.1 (or E10.2) best for pass rule: E10.4 must beat this for blend."""
...
def run_hpo(args: argparse.Namespace) -> int:
    ...
    if not E10_4_BASELINE_PATH.exists() and getattr(args, "run_baseline_if_missing", False):
    ...
    logger.info("=" * 70)
    logger.info("E10.4 XGBoost HPO (vetted features, single model per target)")
    logger.info("  Baseline weighted: %.6f (from %s)", baseline_weighted, E10_4_BASELINE_PATH)
    ...
    def _suggest_xgb_params(trial: optuna.Trial, target_key: str) -> dict:
        p = _default_xgb_ll_params(E10_SEED).copy()
        p["max_depth"] = trial.suggest_int(f"{target_key}_max_depth", 3, 8)
        p["min_child_weight"] = trial.suggest_int(f"{target_key}_min_child_weight", 1, 10)
        p["subsample"] = trial.suggest_float(f"{target_key}_subsample", 0.5, 1.0)
        p["colsample_bytree"] = trial.suggest_float(f"{target_key}_colsample_bytree", 0.5, 1.0)
        p["reg_alpha"] = trial.suggest_float(f"{target_key}_reg_alpha", 1e-4, 2.0)
        p["reg_lambda"] = trial.suggest_float(f"{target_key}_reg_lambda", 0.1, 3.0)
        p["eta"] = trial.suggest_float(f"{target_key}_eta", 0.01, 0.15)
        p["gamma"] = trial.suggest_float(f"{target_key}_gamma", 0.0, 1.0)
        return p
    ...
    def make_objective(study_key: str):
        ...
        def objective(trial: optuna.Trial) -> float:
            n_rounds = trial.suggest_int("num_boost_round", 300, 1200)
            ll_params = {t: _default_xgb_ll_params(E10_SEED) for t in TARGET_COLS}
            ll_params[target] = _suggest_xgb_params(trial, study_key)
            result = run_single_e10_harness_xgb(
                train_df,
                prior_df,
    ...
    for sk in studies:
        logger.info("\n--- E10.4 Optuna study: %s (n_trials=%d) ---", sk, args.n_trials)
        study = optuna.create_study(direction="maximize", study_name=f"e10_4_{sk}")
        ...
        if study.best_trial is not None and study.best_value > baseline_weighted:
            ...
            logger.info("  No improvement over E10.4 baseline for %s", sk)
    ...
    logger.info("\n" + "=" * 70)
    logger.info("E10.4 RESULT")
    logger.info("  E10.4 baseline: %.6f  E10.4 best: %.6f  Reference: %.6f",
    ...
    if best_params:
        ...
        E10_4_BEST_PATH.write_text(json.dumps(out, indent=2), encoding="utf-8")
        logger.info("  Saved best to %s", E10_4_BEST_PATH)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="E10.4: XGBoost dual HPO with vetted features (keep if orthogonal gain in blend)"
    )
